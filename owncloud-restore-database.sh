#!/usr/bin/env bash
# owncloud-restore-database.sh [backup-file-name]
#
# Replaces the ownCloud database with one of the dumps the backups service wrote.
#
#   ./owncloud-restore-database.sh               list and ask
#   ./owncloud-restore-database.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than owncloud.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-owncloud}"
APP_SERVICE="owncloud"
...
DB_NAME="$(env_of OWNCLOUD_DB_NAME)"; DB_USER="$(env_of OWNCLOUD_DB_USER)"; DB_PASS="$(env_of OWNCLOUD_DB_PASSWORD)"
...
if ! docker exec -e MYSQL_PWD="$DB_PASS" "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using ownCloud." >&2
  exit 1
fi
