#!/bin/bash
# End-to-end tests for the owncloud-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-owncloud}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-owncloud-traefik-letsencrypt-docker-compose.yml}"
...
: "${OWNCLOUD_DB_PASSWORD:?set in .env}"
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./owncloud-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./owncloud-restore-database.sh "$(basename "$baseline")" \
...
test_data_restore_roundtrip() {
  ...
  echo "  restoring $(basename "$newest") with ./owncloud-restore-application-data.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./owncloud-restore-application-data.sh "$(basename "$newest")" \
    || { fail "the shipped restore script failed"; return 1; }
